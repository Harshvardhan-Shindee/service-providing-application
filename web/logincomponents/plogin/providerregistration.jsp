            ...
            if ("POST".equalsIgnoreCase(request.getMethod())) {
                ...
                if (name != null && password != null && email != null) {
                    try {
                        ...
                        Provider p = new Provider(name, email, password, phone, aadhar);
                      
                        ProviderDAO pd = new ProviderDAO();
                        int n = pd.registerProvider(p);
                        Provider provider = pd.getProviderByEmailAndPassword(email, password);

                        if (n > 0) {
                            if (provider != null) {
                                // Set session attribute
                                session.setAttribute("provider", provider);
                                out.println("<script>alert('Registration successful! Redirecting to login...'); window.location.href='../../Provider/mainpagep.jsp';</script>");
